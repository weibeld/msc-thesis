...
class NFA
    ...
    def initialize(file)
        ...
        @states = yaml['states'].to_set
        @alphabet = yaml['alphabet'].to_set
        ...
        @accept_states = yaml['accept_states'].to_set
    end          
    ...
    def determinize
        ...
        dfa_states.each do |current_state|
            ...
            @alphabet.each do |symbol|
                ...
                transitions_of_current_state[symbol] = target_state
                dfa_states << target_state unless dfa_states.include? target_state
            end
            dfa_transitions[current_state] = transitions_of_current_state
        end
        dfa_accept_states = Set.new
        dfa_states.each do |s|
            if not (s & @accept_states).empty? then dfa_accept_states << s end
        end
        
        # Create DFA object
        dfa = DFA.new
        dfa.states = dfa_states.to_set
        dfa.alphabet = @alphabet
        dfa.transitions = dfa_transitions
        dfa.start_state = Set[@start_state]
