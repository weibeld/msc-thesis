...
Another way is to use a more classical approach model checking, that leads us back to the fundamental model checking question. In this approach, the system is represented by a Kripke structure $K$, and the property is specified as a formula $\varphi$ of a temporal logic, typically linear temporal logic (LTL). The verification step then consists in checking whether $K$ is a \textit{model} of $\varphi$. This is also expressed as ``$K$ satisfies $\varphi$'', written as $K \models \varphi$, which means very informally that the interpretation $K$ makes the formula $\varphi$ true. It is from this fundamental question that \textit{model checking} has its name.

The model checking test can be done automata-theoretically~\cite{2007_vardi_model_checking} as follows (in Figure~\ref{model_checking}, this would be a sibling to the language containment approach). First, the Kripke structure $K$ is translated to a Büchi automaton $A_K$, then the property formula $\varphi$ is negated and translated to the Büchi automaton $A_{\neg\varphi}$, and finally $\text{\textsf{empty}}(A_K \cap A_{\neg\varphi})$ is tested. This effectively tests $L(K) \subseteq L(\varphi)$, which is equivalent to $K \models \varphi$. By negating the formula $\varphi$, the complementation of the property, that is required for the automata-theoretic verification step, is pushed off from Büchi automata to temporal logic formulas, in which case it is trivial. This approach of model checking is used by the SPIN model checker~\cite{1997_spin}. The disadvantage of this approach is that the typically used temporal logic, LTL, is less expressive than Büchi automata, Hence this approach does not allow to specify many properties that could be expressed by Büchi automata. It has even been stated that the expressivity of LTL is unsufficient for industrial verification applications~\cite{2007_vardi_model_checking}.
...
Worst-case state growths are interesting from a theoretical point of view, but they are poor guides to the actual performance of a construction~\cite{2011_tsai}. For example, if we have a concrete automaton of, say, 15 states that we want to complement with Schewe's construction, the fact that the worst-case state complexity is $(0.76n)^n n^2$ does not reveal anything about how the construction will perform on this concrete automaton. We certainly do not expect the complement to have 1.6 quintillion ($1.6 \times 10^{18}$) states (which would be the worst case), because this would be infeasible in practice anyway.

Furthermore, the fact that, Büchi's construction has a much higher worst-case state growth of $2^{2^{O\left(n\right)}}$ does not mean that it produces a larger complement than Schewe's construction, for this concrete example. It might well be smaller. In fact, worst-case state complexities only allow to adequately deduce something about the specific worst-cases, and not about all the other automata. From a practical point of view, these worst cases are however not interesting, as their application is impracticable anyway~\cite{1995_tasiran} (at least starting from a certain input automaton size).

What we are interested from a practically oriented side is how a construction performs on automata as they could occur in concrete application cases of Büchi complementation, such as automata-theoretic model checking. What is a reasonable complement size to expect for the given automaton with $n$ states? Are there generally easier and harder automata? What are the factors that make an automaton especially easy or hard? How do different constructions compare on specific automata? Are there constructions which are better suited for one type of automata, and other constructions which are better for a different type of automata?

Answers to questions like this can be approached by empirical performance investigations. 


\subsubsection{Review of Empirical Performance Investigations}
...
In 2006, Althoff et al.~\cite{2006_althoff} implemented Safra's~\cite{1988_safra_2} and Muller and Schupp's~\cite{Muller199569} determinisation constructions (that can be used as the base for complementation, see Section~\ref{2_determinisatio-based}) in a tool called OmegaDet, determinsed the Michel automata~\cite{michel1988} with 2 to 6 states, and compared the number of states of the determinised automata.

In 2008, Tsay et al.~\cite{2008_goal_ext} did a comparative experiment with the \goal tool~~\cite{2007_goal}\cite{2008_goal_ext}\cite{2009_goal}\cite{2013_goal} including the constructions by Safra~\cite{1988_safra_2}, Piterman~\cite{2007_piterman}, Thomas~\cite{1999_thomas} (called WAPA\footnote{Via \textbf{W}eak \textbf{A}lternating \textbf{P}arity \textbf{A}utomaton.} in \goal), and Kupferman and Vardi\cite{Kupferman:2001} (called WAA\footnote{Via Weak Alternating Automaton.} in \goal). These constructions are pre-implemented in the publicly available \goal tool\footnote{\url{http://goal.im.ntu.edu.tw/wiki/doku.php}}. As the test data, they used 300 Büchi automata with an average size of 5.4 that have been translated from LTL formulas. They evaluated the number of states and transitions of the complements, and the execution times.

In 2009, Kamarkar and Chakraborty~\cite{2009_karmarkar} proposed an improvement of Schewe's complementation construction~\cite{schewe2009buchi} and implemented it, as well as Schewe's original construction, on top of the model checker NuSMV~\cite{1999_nusmv}\cite{2002_nusmv}. They ran the constructions on 12 test automata and compared the sizes of the output automata with each other. Furthermore, by the means of the \goal tool, they evaluated the constructions by Friedgut, Kupferman, and Vardi~\cite{Kupferman:2001} (called WAA in \goal) and Piterman~\cite{2007_piterman} with the same test data.

In 2010, Tsai et al.~\cite{2011_tsai} carried out the first ``larger-scale'' comparative experiment by using the \goal tool (paper entitled ``State of Büchi Compelentation''). They compared the constructions by Piterman~\cite{2007_piterman}, Schewe~\cite{schewe2009buchi} (called \textit{Rank} in \goal), and Vardi and Wilke~\cite{vardi2007automata} (called \textit{Slice} in \goal),  with different sets of optimsiations. As the test data, they use 11,000 randomly generated automata with 15 states and an alphabet size of 2, that are split into 110 classes of 11 transition densities and 10 acceptancd densities.

In the same year, 2010, Breuers~\cite{2010_breuers_bsc} proposed an improvment for the construction by Sistla, Vardi, and Wolper~\cite{PrasadSistla1987217},  implemented it, and tested it on randomly generated test sets of ``easy'', ``medium'', and ``hard'' automata of sizes 5, 10, and 15, and an alphabet size of 2. The difficulty classes of the test automat are based on different combinations of transition and acceptance densities. The generation of the test automata is inspired by the experiments by Tsai et al.~\cite{2011_tsai}.

In 2012, Breuers et al.~\cite{2012_breuers} implemented their improvement of Sistla, Vardi, and Wolper's construction~\cite{PrasadSistla1987217} as the publicly available tool Alekto\footnote{\url{http://www.automata.rwth-aachen.de/research/Alekto/}}, and tested it on the test set 11,000 automata used by Tsai et al.~\cite{2011_tsai}. They compare the performance of their construction on this test set with the results of the three constructions evaluated and reported in~\cite{2011_tsai}
