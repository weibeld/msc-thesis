...
At this point we will start using a notation for these different types of \om-automata, including Büchi automata, that has been introduced in~\cite{2011_tsai}. It consists of a three-letter acronyme where the first letter is either ``D'' or ``N'' specifying whether the automaton is deterministic or non-deterministc. The second letter is the first letter of the automaton type, that is ``B'' for Büchi, ``M'' for Muller, ``R'' for Rabin, ``S'' for Streett, and ``P'' for parity automata. The third letter is always ``W'' meaning that the automaton runs on classical words, rather than, for example, trees.

Thus, throughout this thesis we will use DBW for deterministic Büchi automata, NBW for non-deterministic Büchi automata, DMW for deterministic Muller automata, and so on.

Regarding the expressivity of Muller, Rabin, Streett, and parity automata, they are all equivalent to the \om-regular languages, in both, their deterministic and non-deterministic forms~\cite{Thomas:1991}. This means that NBW, DMW, NMW, DRW, NRW, DSW, NSW, DPW, and NPW are equivalent, and it is just DBW, the deterministic Büchi automata, that have a different expressivity.
...
\section{History of Büchi Complementation}
