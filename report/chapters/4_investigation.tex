...
The first version is the plain Fribourg construction without any optimisations or options. The next two versions are devoted to investigate the R2C optimisation. Version 2, Fribourg+R2C, applies the optimsation only to the automata which happen to be complete. On the other hand, Version 3, Fribourg+R2C+C, makes all input automata preliminarily complete by adding a sink state, so that the R2C optimisation  can be applied to \it{all} automata. The question here is, does it pay off to increase the size of automata by one (for adding a sink state) but then being able to apply R2C, over not increasing the size of the automata but not being able to apply R2C? As we have seen in Section~\ref{goal_testset}, only 9\% of the automata of the GOAL test set are complete. This means that Version 2 actually lets 91\% of the test set unaffected, whereas Version 3 would increase the size of, and thus generally make more difficult, these 91\% of the automata.



 can R2C compensate for the increase by one of the size of the input automata that results from adding a sink state to make them complete? As we have seen in Section~\ref{goal_testset}, only 9\% of the automata of the GOAL test set are complete, thus 91\% of the test set would be affected by the C option.



The exact set of versions of the Fribourg construction that we chose for our internal tests are the following. 
