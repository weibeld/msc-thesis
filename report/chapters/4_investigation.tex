...
Regarding the dependencies of the M1 and M2 optimisation, there are only two cases we can test. First, M1 alone, and second, M1 and M2 together. Assuming that the R2C optimisation adds a certain performance gain on top of an existing construction, we can then combine the better one of \id{M1} and \id{M1+M2} with R2C. We can already reveal at this point that \id{M1} performs slightly better than \id{M1+M2} on our test set, even though \id{M1+M2} has a better theoretical worst-case complexity. This topic is further discussed in Chapter~\ref{chap_results}. Thus, the version that we will want to test is \id{M1+R2C}.
...
\begin{enumerate}
\item \id{Fribourg}
\item \id{Fribourg+R2C}
\item \id{Fribourg+R2C+C}
\item \id{Fribourg+M1}
\item \id{Fribourg+M1+M2}
\item \id{Fribourg+M1+R2C}
\item \id{Fribourg+M1+R2C+MACC+R}
\end{enumerate}
...
\item long.q
  ...
  \item h_cpu limit: >200:00:00 (not sure)
  \end{itemize}
\item mpi.q
  \begin{itemize}
  \item hnode 01--42
  ...
  \item 64 GB RAM
  \item $\rightarrow$ 4 GB RAM per core (slot)
  \item h_cpu limit: 72:00:00
  \item h_rt limit: 73:00:00
  \end{itemize}
\item highmem.q
  \begin{itemize}
  \item jnode 01--21
