...
\caption{Measured execution times for the Michel automata 1 to 4. These automata have 3, 4, 5, and 6 states, respectively.}
\label{i.m.times}
\end{table}
...
\label{5_external}
The external tests consist of the comparison of the best version of the Fribourg construction on each test set, with three other complementation constructions. These constructions are the following (see Section~\ref{4_external}):
...
On the other hand, we declared the best versions of the Fribourg construction on the two test sets to be:
...
In the following two sections, we first present the results of these tests on the \goal{} test set (Section~\ref{5_external_goal}), followed by the results on the Michel test set (Section~\ref{5_external_michel}).
...
- identification of easy, medium and hard automata
- evidence that the worst-case state complexity does not allow to conclude on the performance of the construciton on concrete cases
  - examples
    - Friburg+M1 and Fribourg+M1+M2
    - Rank on Michel test set
- extreme differences in execution times, complement sizes (especially for Michel test set)
