...
Another limitation of the test data are the characteristics of the automata in the \goal{} test set itself. We have identified that 61.8\% of all automata in the \goal{} test set are universal (see Section~\ref{4_goal_testset}). Of the 110 transition density/acceptance density classes, 30 classes even contain 95\% or more universal automata (see matrix in Figure~\ref{testset_analysis} (b)). Universal automata are a special case of automata, and we do not know if and how their high representation affects our results. Furtheremore, In the practical application of Büchi complementation, it might be rarely the case that we have to complement a universal automata. For example, in automata-theoretic model checking, we have to complement the automaton representing the property to verify, and it is unlikely that this automaton is universal. Consequently, the bias of the \goal{} test set towards universal automata might affect the significance of our study to the practical applications of Büchi complementation.

Regarding the Michel test set, we were only able to include the first four Michel automata with up to 6 states (Michel~1 to~4). This might not be enough to represent the behaviour of the tested constructions on Michel automata in general. For example, we determined a superiority of Rank and an inferiority of Piterman on the tested Michel automata. However, to conclude that this is likely to be the case for Michel automata in general, we would require results from more Michel automata.

Generally, our test data consists of rather artificial examples, and is not based on real-world complementation tasks. The automata that need to be complemented, for example, in automata-theoretic model checking, are likely to have more than 15 states and more than 2 alphabet symbols. The automata that have been used by Tasiran et al.~\cite{1995_tasiran} for representing model checking properties of the form ``if $X$, then eventually $Y$'' have several hundreds of states, and according to the authors, these are still \textit{small} property automata. As mentioned, we do not know if and how our results from automata with 15 states can be generalised to automata with several hundreds of states.

\subsubsection{Reliance on Implementation}
Naturally, the results of our empirical investigations rely on implementations of the involved constructions. Regarding the constructions that are pre-implemented in \goal, we do not have any details of their implementation (as \goal{} is not open-source). Concerning the correctness, we know that the constructions are cross-checked by their authors~\cite{2008_goal_ext} (in a similar way as we checked the correctness of our implementation of the Fribourg construction), and we think that it is reasonably to assume that they are correct, in the sense of producing correct complements. However, we do not know if and to what extent the implementations stick to the original specifications of the constructions. That means, for example, does Piterman in \goal{} truly represent Piterman's original constructions as described in~\cite{2006_piterman,2007_piterman}\footnote{We do not question the authenticity of the implementation of this specific construction, nor the authenticity of all the constructions implemented \goal. Rather, we see the problem of faithful representation as a general issue in studies that rely on implemenations of algorithms that were previously only theoretically described.}? Furthermore, the implementations in \goal{} include diverse options, and it is not always clear how these options are specified and implemented. We do not know if and how such variations affect the behaviour, and in the end the performance, of a construction. Therefore, mapping the results of a specific implementation back to a  construction, as described in the literature, has to be done with care.

Another limitation that results from the reliance on implementations is that other measures than the complement size, such as the execution time or memory usage, are not reliably comparable. A construction can be implemented more or less efficiently, which influences metrics such as execution time or memory usage. Consequently, if a construction is faster at a specific task than another, we cannot tell with certainty that it is more efficient in terms of speed than the other construction, because the difference could be caused by the implementation. This, and the fact that the execution time measurements are generally not completely reliable, is the reason that we use the execution times only as an auxiliary metrics, and to give an idea of the involved time requirements.
...
For the Fribourg construction, we first evaluated different versions against each other (internal tests), and then chose the best one on each test set for comparing it with other constructions (external tests). However, for the other constructions, we did not do such an evaluation, but selected the version that is defined by the default options. Consequently, we do not know whether these versions are also the best ones for the respective test sets. A fairer setup would compare only the best version of each construction against each other. If, the other way round, we would compare the least performant version of the Fribourg construction on the two test sets, Fribourg, to the other constructions, then the Fribourg construction would occupy the last place in most rankings. One has to keep in mind such differences when concluding from the results of a specific version to a construction per se.
...
The statistical evaluations that we did for the results of our investigations (especially of the \goal{} test set) are far from exhaustive. It is likely that a more in-depth analyses would lead to new or different conclusions. For example, for the \goal{} test set, we considered only aggregated statistics, such as the mean or the median. A more detailed evaluation could include considering individual automata. For example, we could determine for which specific automata a construction produces smaller complements than another construction. In this way, the relative strengths and weaknesses of different constructions could be analysed more reliably. This would be insightful, for example, for the relations of Fribourg+M1+M2 and Fribourg+M1, or Fribourg+R2C+C and Fribourg, for which we already identified that there are relative differences based on automata characteristics.

Furthermore, such analyses would allow find out whether the automata that are hard for one construction are the same that are hard for another construction. By considering only aggregated statistics, such as the median, this is in principle not possible. For example, if two constructions have the same median complement size for a given transition density/acceptance density class of the \goal{} test set, then it could still be that a specific automaton results in a large complement with one construction, but a small one with another construction. Altogether, such investigations would enhance our understanding of the behaviour of these constructions, what would in turn support the portfolio approach of selecting suitable constructions for specific automata.
