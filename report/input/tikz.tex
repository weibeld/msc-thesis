...
\tikzset{
  ...
  tree/.style={
    every tree node/.style={my state},
...
\node[anchor=base,xshift=-1.5cm] at (m.base){$A=$};  
\end{tikzpicture}
}
...
\node[frame,anchor=east,xshift=0.7mm] (level2) at (m.east) {};
...
\node (arrow0) at (level0.east) {$\rightarrow$};
\node (arrow1) at (level1.east) {$\rightarrow$};
\node (arrow2) at (level2.east) {$\rightarrow$};
\end{scope}
