...
# Fake local scratch in global home
TMP_2=not_scratch
mkdir $TMP_2

# Copy data to local scratch
cp $data_archive $TMP_2/tmp.tar.gz
data=$TMP_2/data
mkdir $data
tar xzf $TMP_2/tmp.tar.gz -C $data --strip-components 1 && rm $TMP_2/tmp.tar.gz

# Copy GOAL to local scratch
cp $goal_archive $TMP_2/tmp.tar.gz
goal_dir=$TMP_2/GOAL
mkdir $goal_dir
tar xzf $TMP_2/tmp.tar.gz -C $goal_dir --strip-components 1 && rm $TMP_2/tmp.tar.gz
goal=$goal_dir/goal
...
$goal generate -s 1 >$TMP_2/tmp.gff
err=$($goal complement -m $algo $TMP_2/tmp.gff 2>&1 1>/dev/null)
if [ -n "$err" ]; then echo "GOAL error: $err"; exit 1; fi
rm $TMP_2/tmp.gff

# Out file and log file
out=$TMP_2/$JOB_NAME.out
log=$TMP_2/$JOB_NAME.log
...
stdout=$TMP_2/tmp.stdout
stderr=$TMP_2/tmp.stderr
