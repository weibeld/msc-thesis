...
states/.style={
  ...
  st/.style={my state},
  acc/.style={my state,my accepting},
},
tree/.style={
  states,
...
\automaton
\hfill
\begin{tikzpicture}[tree,s/.style={my state},a/.style={my state,my accepting}]
\Tree
[.\node[s]{\qq0}; [.\node[a]{\qq1};  [.\node[s](m){\qq2}; [.\node[s]{\qq2}; \node[s]{\qq2}; ] ] ]
              [.\node[s]{\qqq02};   \node[a]{\qq1};
                             [.\node[s]{\qq0};      \node[a]{\qq1};
...
\symbols
\end{center}
\caption{XXXXXXX Automaton $A$ and the first five levels of the left-to-right reduced split tree of the runs of $A$ on the word $a^\omega$.}
\end{figure}
