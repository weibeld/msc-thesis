...
We mentioned that the greedy run somehow represents the deleted runs. More precisely, the relation is as follows and has been proved in~\cite{vardi2007automata}: if any of the deleted runs is a prefix of a run that is Büchi-accepted (that is, an infinite run visiting infinitely many accepting $A$-states), then the greedy run is so too. That means that if the greedy cannot be expanded to a Büchi-accepting run, then none of the deleted runs could be either. Conversely, if any of the deleted runs could become Büchi-accepting, then the greedy run can so too. So, the greedy run is sufficient to indicate the existence or non-existence of a Büchi-accepting run with this prefix, and it is safe to delete all the other runs.
 ...
 The difference is that in a reduced split tree every $A$-state occurs at most once on every level. All the other occurrences of this state are then discarded. To allow this, however, there is a restriction on the order of the accepting and non-accepting child of every vertex. Either the accepting child must always be to the left of the non-accepting child, or vice versa. In the former case, we call the tree a left-to-right reduced split tree, in the latter case a right-to-left reduced split tree. The tree shown in Figure~\ref{reduced_split_tree} is a right-to-left reduced split tree.



 This means that in a reduced split tree, some runs are discarded, in particular runs that lead to the same state on the same level. For example, in the split tree in Figure~\ref{split_tree}, there are four runs from \q0 to \q2 on the fifth level. In the reduced split tree in Figure~\ref{reduced_split_tree}, however, there is only one run from \q0 to \q2 on the fifth level. The other three runs have been discarded.
...
To construct level $n+1$ based on level $n$ of a right-to-left reduced split tree, one has to process all the vertices of level $n$ from right to left. For each vertex one creates the accepting and non-accepting child, as in the construction of the normal split tree, but then removes from these two sets of $A$-states all the $A$-states that already occur in some of the existing vertices to the right on level $n+1$. Then, the accepting child is put to the right of the non-accepting child on level $n+1$.
...
If we look closer at $B$'s run on \aom, we see that the accepting $A$-state \q1 is indeed visited infinitely often by the unification of $A$-runs that the $B$-run represents (\q1 occurs on every but the first level of the run tree in Figure~\ref{run_tree}). However, the conclusion that in this case there is an $A$-run that visits \q1 infinitely often (the Büchi acceptance condition) is wrong. The infinite visits of a $B$-run to an accepting $A$-state $q$ can have three reasons.
\begin{itemize}
\item A finite or infinite number of $A$-runs visits $q$ infinitely often each
\item A infinite number of $A$-runs visits $q$ finitely often each
\end{itemize}
In the first case it is correct to accept the $B$-run because there must be an $A$-run that visits $q$ infinitely often.
