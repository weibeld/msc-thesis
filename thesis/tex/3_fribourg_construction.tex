...
\subsubsection{The Steps for Constructing the Lower Part}
...
Generally, the relation between $A$ and its complement $B$ is that a word $w$ is accepted by $B$ if and only if all the runs of $A$ on $w$ are rejecting. Since this condition is on \emph{all} runs of $A$, the construction somehow has to keep track of them. 
